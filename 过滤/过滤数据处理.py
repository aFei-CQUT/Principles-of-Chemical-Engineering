...
if __name__ == '__main__':
    ...
    for i in range(3):
        ...
        # 添加拟合方程表达式
        equation_text = f'y = {k_value:.4f} * x + {q_e_intercept:.4f}'
        ...
        plt.savefig(f'./过滤拟合图结果/{2*i+1}.png')
        plt.show()
        ...
        if len(outliers) > 0:
            ...
            # 添加拟合方程表达式
            equation_text_refit = f'y = {refit_slope:.4f} * x + {refit_intercept:.4f}'
            ...
            plt.savefig(f'./过滤拟合图结果/{2*i+2}.png')
            plt.show()
...
# 保存图像并移除多余的边框
plt.savefig(r'./过滤拟合图结果/拟合图整合图.png', bbox_inches='tight')
