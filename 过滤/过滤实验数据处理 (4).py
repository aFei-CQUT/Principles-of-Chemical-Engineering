...
# 定义函数：拟合直线
def filter_fit_line_without_last_point(selected_data):
    # 滤膜参数
    a = 0.0475
    deltaV = 9.446 * 10**-4
    deltaQ = deltaV / a
    ...
    # 去掉最后一个点进行线性回归拟合
    fit_coordinates = fit_coordinates[:-1]
...
def filter_plot(selected_data_list, fit_coordinates_list, fit_line_list, delta_theta_over_delta_q_list_list, save_path):
    ...
    deltaV = 9.446 * 10**-4
    deltaQ = deltaV / a
    q_list = np.linspace(0.05, 0.05 + len(selected_data_list[0]) * deltaQ, len(selected_data_list[0]) + 1)
    ...
    for i in range(len(selected_data_list)):
        ...
        plt.plot(fit_coordinates_list[i][:, 0], fit_line_list[i].predict(fit_coordinates_list[i][:, 0].reshape(-1, 1)), label='去掉最后一个点的拟合线' + str(i+1))
...
for i in range(3):
    ...
    # 拟合直线
    k_value, q_e_intercept, fit_coordinates, model, delta_theta_over_delta_q_list = filter_fit_line_without_last_point(selected_data)
    ...
    # 输出拟合结果
    print("去掉最后一个点的拟合直线斜率：", k_value)
    print("去掉最后一个点的拟合直线截距：", q_e_intercept)
